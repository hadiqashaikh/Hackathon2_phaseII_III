...
def chat_message(
    ...
    try:
        ...
        try:
            logger.info(f"Creating OpenRouterAgent with user_id: '{user_id}' (type: {type(user_id).__name__})")
            # Ensure user_id is a clean string
            clean_user_id = str(user_id).strip() if user_id else "debug-user"
            logger.info(f"Using clean_user_id: '{clean_user_id}'")
            agent = OpenRouterAgent(user_id=clean_user_id)
